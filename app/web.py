...
def sec():
    ...
    if chk.checkExist() is True:
        return render_template(
            "sec.html", uid=request.args["uid"], mytitle="Reveal secret"
        )
    else:
...
def get():
    if request.method == "POST":
        ...
        if secret.decrypt() is False:
            flash("Incorrect credentials or secret might be destroyed")
            return redirect(url_for("index"))
        elif secret.decrypt() is not False:
            result = secret.decrypt()
            secret.destroy()
            return render_template(
                "get.html", secret=result, mytitle="Retrieve Secret"
            )
        else:
